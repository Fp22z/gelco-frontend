...
function ModalDetalle({ producto, onClose }) {
  ...
  const descuentoPct = producto.enOferta 
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-detalle" onClick={e => e.stopPropagation()}>
        ...
        {/* Envoltura para Badges flotantes + Imagen */}
        <div className="modal-img-container">
          ...
          {producto.imagenUrl ? (
            <img src={producto.imagenUrl} alt={producto.nombre} className="modal-img" />
          ) : (
            <div className="modal-img-placeholder">{emoji}</div>
          )}
        </div>
        ...
        <div className="modal-body">
          ...
          <div className="modal-meta">
            {/* Precios agrupados a la izquierda */}
            ...
            {/* Stock con el estilo de la píldora a la derecha */}
            <div className={`modal-status-pill ${status.cls}`}>
              <span className="status-dot" />
              {status.label} • {producto.stock} uds.
            </div>
          </div>

          <div className="modal-actions">
            <button className="btn-modal-cerrar" onClick={onClose}>Cerrar</button>
            <button className="btn-modal-anadir" disabled={status.cls === 'agotado'}>
...
function ProductoCard({ producto, onClick }) {
  ...
  // Calculamos el porcentaje de descuento si está en oferta
  const descuentoPct = producto.enOferta 
    ? Math.round(((producto.precio - producto.precioOferta) / producto.precio) * 100) 
    : 0;
  ...
  return (
    <div
      className={`producto-card${status.cls === 'agotado' ? ' agotado' : ''}`}
    ...
    >
      <div className="producto-img-wrap">
        ...
        {/* Aquí entra la magia de la imagen */}
        {producto.imagenUrl ? (
          <img src={producto.imagenUrl} alt={producto.nombre} className="producto-img" />
        ) : (
          <div className="producto-img-placeholder">{emoji}</div>
        )}
      </div>
      ...
      <div className="producto-info">
        ...
        <div className="producto-precio-wrap">
          ...
          {/* Contenedor para el precio original y el % de descuento */}
          {producto.enOferta && (
        ...
        <span className={`producto-status ${status.cls}`}>
          <span className="status-dot" />
          {status.label}
        </span>
      </div>
...
export default function Catalogo() {
  ...
  const precioMax = useMemo(() =>
    productos.length ? Math.ceil(Math.max(...productos.map(p => p.precio))) : 200
  , [productos]);
  ...
  const productosFiltrados = useMemo(() => {
    ...
    if (categoriaActiva !== 'Todas')
      lista = lista.filter(p => (p.categoriaNombre ?? 'Sin categoría') === categoriaActiva);
    if (marcasActivas.size > 0)
      lista = lista.filter(p => marcasActivas.has(p.marcaNombre));
    if (lineasActivas.size > 0)
      lista = lista.filter(p => lineasActivas.has(p.lineaNombre));
    if (soloEnDescuento)
      lista = lista.filter(p => p.enOferta);
    if (soloEnCampania)
      lista = lista.filter(p => p.enCampania);
    if (soloConStock)
      lista = lista.filter(p => p.stock > 0 && p.activo);
    ...
    return lista;
  }, [productos, busqueda, categoriaActiva, marcasActivas, lineasActivas,
      soloEnDescuento, soloEnCampania, soloConStock, maxPrecio, ordenar]);
  ...
  const hayFiltrosActivos = categoriaActiva !== 'Todas' || marcasActivas.size > 0
    || lineasActivas.size > 0 || soloEnDescuento || soloEnCampania || soloConStock;

  const limpiarFiltros = () => {
    setCategoriaActiva('Todas');
    setMarcasActivas(new Set());
    setLineasActivas(new Set());
    setSoloEnDescuento(false);
    setSoloEnCampania(false);
    setSoloConStock(false);
    setMaxPrecio(precioMax);
  };
    ...
    <div className="catalogo-page">
      ...
      <aside className="catalogo-sidebar">
        ...
        <div className="filtro-section">
          <h4>Categorías</h4>
          <div
            className={`categoria-item${categoriaActiva === 'Todas' ? ' active' : ''}`}
          ...
          {Object.entries(categorias).map(([nombre, count]) => (
            <div
              key={nombre}
              className={`categoria-item${categoriaActiva === nombre ? ' active' : ''}`}
        ...
        {Object.keys(marcas).length > 0 && (
          <div className="filtro-section">
            ...
            {Object.entries(marcas).map(([nombre, count]) => (
              <CheckItem
                key={nombre} label={nombre} count={count}
                checked={marcasActivas.has(nombre)}
                onChange={() => toggleSet(setMarcasActivas, nombre)}
              />
            ))}
          </div>
        )}
          ...
          <div className="filtro-section">
            ...
            {Object.entries(lineas).map(([nombre]) => (
              <CheckItem
                ...
                checked={lineasActivas.has(nombre)}
                onChange={() => toggleSet(setLineasActivas, nombre)}
              />
            ))}
          </div>
        )}
        ...
        <div className="filtro-section">
          ...
          <div className="precio-range">
            <div className="precio-labels">
              <span>S/. 0</span><span>S/. {precioMax}</span>
            </div>
            <input type="range" min={0} max={precioMax || 200} value={maxPrecio}
      ...
      <div className="catalogo-main">
        ...
        <div className="catalogo-grid-area">
          ...
          {!loading && !error && productosFiltrados.length === 0 && (
            <div className="catalogo-empty">
              ...
              {hayFiltrosActivos && (
                <button className="btn-limpiar-inline" onClick={limpiarFiltros}>Limpiar filtros</button>
              )}
            </div>
          )}
          {!loading && !error && Object.entries(porCategoria).map(([categoria, prods]) => (
            <div key={categoria} className="catalogo-section">
              ...
              <div className="catalogo-grid">
                {prods.map(p => (
                  <ProductoCard key={p.id} producto={p} onClick={setProductoSeleccionado} />
                ))}
              </div>
            </div>
          ))}
        ...
        <div className="catalogo-footer">
          <label className="solo-stock-check">
      ...
      {productoSeleccionado && (
        <ModalDetalle producto={productoSeleccionado} onClose={() => setProductoSeleccionado(null)} />
      )}
    </div>
  );
}